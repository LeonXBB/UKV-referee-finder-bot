local = {
    "proposition_to_log_in": "Вітаємо! Будь ласка, авторизуйтесь з кодом, який був наданий адміністрацією!",
    "log_in_as_referee_button_text": "Авторизуватися як рефері",
    "log_in_as_representitive_button_text": "Авторизуватися як представник",
    "failed_log_in": "Авторизація не вдалася! Будь ласка, спробуйте ще раз.",
    "main_menu": "Вітаємо, {}! Будь ласка, оберіть потрібну дію!",
    "log_out_button": "Вийти з акаунту",
    "see_referees_list_button": "Переглянути список рефері",
    "see_my_team_future_games_button": "Переглянути майбутні матчі команди {}",
    "love_referee": "😁",
    "hate_referee": "😡",
    "dont_care_referee": "😒",
    "successfully_applied_changes": "Зміни успішно збережені",
    "return_to_main_menu_text": "Натисність, щоб повернутися до головного меню",
    "return_to_main_menu_button": "Повернутися",
    "match_template": "\n     <b>{}</b> - <b>{}</b>     \n   <i>{}</i>   \n   <i>{}</i>",
    "match_template_with_referees": "\n     <b>{}</b> - <b>{}</b>     \n   <i>{}</i>   \n   <i>{}</i>\nПерший суддя: {}\nДругий суддя: {}\nСеккретар: {}",
    "referee_not_found": "???",
    "referees_titles": ("Перший суддя: {}", "Другий суддя: {}", "Секретар: {}"),
    "referee_titles_empty": ("Перший суддя", "Другий суддя", "Секретар"),
    "look_for_referee_button": "Знайти суддю",
    "cancel_agreement_button": "Скасувати домовленність",
    "request_template": "Мінімальна категорія: {}\nГонорар: {}\nТрансфер: {}",
    "categories_titles": ("4 кат.", "3 кат.", "2 кат.", "1 кат."),
    "pay_titles": ("Звичайний", "Підвищений"),
    "transfer_titles": ("Відсутній", "Наявний"),
    "send_request": "Надіслати запит",
    "successfully_sent_request": "Запит надіслано. Ми повідомимо, коли хтось з суддів погодиться.",
    "match_request": "Пошук судді, матч {} - {}\n\n{}\n{}\n\nРоль: {}\n\nГонорар: {}\nТрансфер: {}",
    "agree_to_request": "Погодитись",
    "deny_request": "Відмовитись",
    "cancel_request_button": "Скасувати запит",
    "see_my_future_games_button": "Переглянути свої майбутні матчі",
    "thank you": "Дякуємо! Інформація передана представнику команди для погодження",
    "sorry_already_taken": "Вибачте, інший суддя вже погодився на цей запит.",
    "no_games_yet": "Майбутні матчі ще не призначені.",
    "received_acceptance_of_the_request": "Суддя {} погодився на запит з приводу матчу {} - {}\n\n{}\n{}\n\nРоль: {}",
    "final_acceptance_staff": "Дякуємо за погодження! До зустрічі на майданчику!",
    "final_acceptance_referee": "Представник команди {} підтвердив Вашу домовленність! До зустрічі на майданчику!",
    "final_acceptance_declined": "На жаль, представник команди {} не погодив Вашу кандідатуру. Успіхів!",
    "request_cancelled": "Запит скасовано.",
    "agreement_cancelled": "Домовленність скасовано.",
    "agreement_cancelled_by_referee": "Ваша домовленність була скасована рефері.",
    "agreement_cancelled_by_staff": "Ваша домовленність була скасована представником команди {}"
}